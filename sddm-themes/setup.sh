...
## Based on original by Gaming Evolution Centre https://github.com/GamingEvolutionCentre/Linux-Stuff/SDDM-themes
## Copyright (c) 2019-present, Gaming Evolution Centre

# Script works in Arch

set -euo pipefail

readonly THEME_REPO="https://github.com/GamingEvolutionCentre/Linux-Stuff.git"
readonly THEME_SOURCE_DIR="SDDM-themes"
...
clone_repo() {
    if [[ -d "$PATH_TO_GIT_CLONE" ]]; then
        local backup_path="${PATH_TO_GIT_CLONE}_backup"
        [[ -e "$backup_path" ]] && backup_path="${backup_path}_$DATE"
        spin "Backing up existing SDDM themes..." cp -a "$PATH_TO_GIT_CLONE" "$backup_path"
...
# Install theme
install_theme() {
    local src="$PATH_TO_GIT_CLONE/$THEME_SOURCE_DIR"
