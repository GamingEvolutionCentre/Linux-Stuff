...
uninstall_dependencies() {
	...
	if [ "$PACKAGER" = "brew" ]; then
		brew uninstall --ignore-dependencies bash-completion bash-completion@2 bat tree multitail fastfetch neovim trash fzf zoxide || true
...
restore_terminal_font() {
	...
	FONT_BACKUP_DIR="$USER_HOME/.config/bash"
