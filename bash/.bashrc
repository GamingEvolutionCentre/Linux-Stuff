...
#######################################################
# Gaming Evolution Centre's Bashrc
#######################################################
	...
	for _bash_completion in \
		...
		if [ -f "$_bash_completion" ]; then
			if [ "$_bashrc_os" = "Darwin" ] && [ "${BASH_VERSINFO[0]}" -lt 4 ]; then
				_bashrc_completion_dir="$MYBASHDIR/bash_completion.d"
...
export MYBASHDIR="$HOME/.config/bash"
...
ebrc() {
	"${EDITOR:-vi}" "$MYBASHDIR/.bashrc"
}

# Show help for this .bashrc file
hlp() {
	if [ -f "$MYBASHDIR/.bashrc_help" ]; then
		less "$MYBASHDIR/.bashrc_help"
	else
		echo "No $MYBASHDIR/.bashrc_help file found."
	fi
}
