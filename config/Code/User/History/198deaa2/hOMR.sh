...
whiptail --title "Hyprland Install Script" \
  --msgbox "Welcome to Hyprland Install Script!!!\n\n\
...
while true; do
  ...
  if [[ "$dots_selected" == "OFF" ]]; then
    ...
    if ! whiptail --title "Hyprland Dot Files" --yesno \
      "You have not selected to install the pre-configured Hyprland-Dots dotfiles.\n\nKindly NOTE that if this a *new*  install, not an *upgrade*,  and you proceed without Hyprland-Dots will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without Hyprland Dots or return to choices/options?" \
...
for option in "${options[@]}"; do
  ...
  sddm_theme)
    ...
    ;;
    bootloader_theme)
    echo "${INFO} Downloading & Installing ${SKY_BLUE}Additional bootloader theme...${RESET}" | tee -a "$LOG"
    execute_script "bootloader.sh"
    ;;
  zsh)
  ...
  dots)
    ...
    echo "${INFO} Installing pre-configured ${SKY_BLUE}Hyprland dotfiles...${RESET}" | tee -a "$LOG"
...
if pacman -Q hyprland &>/dev/null || pacman -Q &>/dev/null; then
  printf "\n ${OK} 👌 Hyprland is installed. However, some essential packages may not be installed. Please see above!"
  printf "\n${CAT} Ignore this message if it states ${YELLOW}All essential packages${RESET} are installed as per above\n"
  sleep 2
  printf "\n%.0s" {1..2}

  printf "${SKY_BLUE}Thank you${RESET} 🫰 for using 🇵🇭 ${MAGENTA}Hyprland Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
  printf "\n%.0s" {1..2}
