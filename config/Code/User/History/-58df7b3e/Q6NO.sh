...
cd ~/Downloads/neovim-plugin-bundle-builder/plugins || exit 1
...
Offline Neovim plugin bundle.

The plugins/ directory contains the plugin source trees with .git removed.
You can keep this ZIP as a frozen backup or copy the folders into the
plugin location used by your Neovim package manager.

Note: blink.cmp is deliberately downloaded from its stable v1 branch.
Its current main branch is v2 development and requires Neovim 0.12+
plus the separate blink.lib dependency.
EOF

ZIP_NAME="$(pwd)/neovim-plugins-offline.zip"
rm -f "$ZIP_NAME"
(cd "$ROOT" && zip -qr "$ZIP_NAME" .)

echo
echo "Done: $ZIP_NAME"
echo "The ZIP contains all plugin files with Git metadata removed."
